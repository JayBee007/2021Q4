...
fn main() {
    ...
    let upper_left = parse_complex(&args[3]).expect("error parsing uper left corner point");
    let lower_right = parse_complex(&args[4]).expect("error parsing uper left corner point");
